...
var DisplayRepositoryBox=React.createClass({
	handleAddRepository(Category, repoId, repoObject){		
		$.ajax({
			url:"http://localhost:8342/repos/AddRepositories",
			type:'POST',
			data:{"repoID":repoId,
			"Name":repoObject.name,
			"Access":repoObject.private,
			"Stars":repoObject.stargazers_count,
			"Category":Category,
			"Avatar":repoObject.owner.avatar_url,
			"Description":"hi description"},
			success:function(data){
				console.log(data+"add repo success");
				alert(data);
			},
			error:function(err){
				console.log(err+"add repo fail");
			}
		});
	...
	render(){
		...
		if(searchOption==="UserName"){
			...
			<br></br>
			 <div className="row">
			<div className="col-lg-10">
			 <a href={RepoObject.html_url} target="_blank" role="button" className="btn btn-info"> View Repository on Github <span className="glyphicon glyphicon-new-window"></span> </a>&emsp;&nbsp;&emsp;
			<button className="btn btn-primary" data-toggle="modal" data-target={'#'+RepoObject.id}> Add to Favourites <span className="glyphicon glyphicon-star"></span> </button>
			</div>
			</div>
			</div>
			<br></br>
